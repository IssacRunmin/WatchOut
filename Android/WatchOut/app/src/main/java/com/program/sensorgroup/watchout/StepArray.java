...
public class StepArray {
    protected static int MaxStepsNum = 25; //the max Steps that logs 600
    ...
    public static synchronized boolean LogData(int ToStep){
        // To Step is the real step, So as the LastStepLog
        ...
        try {
            while (ToStep >= LastStepLog) {
                CircleStep = LastStepLog % MaxStepsNum;
                StepData LogStep = Steps[CircleStep];
                OutString = "";
...
//                }
                OutString += "\r\n";
                try{
                    WatchOutService.StepDataOut.write(OutString.getBytes());
                }
                catch (IOException e){
                    e.printStackTrace();
                }
...
//            textView_filenotice.setText("Err: StepData file Write");
            Log.d("StepArray","Failed to log");
            return false;
        }
        return true;
    }
    protected synchronized static int HMM(int count){
        // count is the real step count
        int Observation[];
...
//            Observation[i] = StepArray.HMMSeq[i];
        ...
        if (!Circle) {
            ...
            System.arraycopy(HMMSeq, ToStep+1, Observation, 0, MaxStepsNum - ToStep - 1);
            System.arraycopy(HMMSeq, 0,Observation,MaxStepsNum - ToStep - 1 ,ToStep + 1);
        }
        HMMOutState = Viterbi.compute(Observation, MyLib.HMM_States, HMM_Pi, HMM_Trans, HMM_Emit);
        return Circle? HMMOutState[MaxStepsNum - 1] : HMMOutState[ToStep-1];
//        int i;
//        final int templen=5;
...
//            LastStep.State = 0;
            ...
            LastStep.Event = MyLib.EVENT_STAND;
            Steps[StepCount % MaxStepsNum] = LastStep;
        }
    }
