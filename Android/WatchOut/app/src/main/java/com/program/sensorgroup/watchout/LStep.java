...
public class LStep extends Thread {
    ...
    public LStep(double StancePitch, double StanceYaw, double maxAccMag,
        ...
        LStep.StancePitch[index]=StancePitch;
        LStep.StanceYaw[index]=StanceYaw;
        LStep.maxAccMag[index]=maxAccMag;
        LStep.StanceLength=(StanceEndIndex-StanceStartIndex+MOD)%MOD;
        LStep.StepLength=(StepEndIndex-StepStartIndex+MOD)%MOD;
        pitchStatic = StancePitchIn;
        this.output=output;
        LStep.long_deltaTime=long_deltaTime;
        this.start();
    }
    public void run(){
        try {
