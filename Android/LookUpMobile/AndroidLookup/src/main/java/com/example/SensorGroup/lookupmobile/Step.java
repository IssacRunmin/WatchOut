...
public class Step extends Thread{
    ...
    private static int MaxPointNum = 600;
    ...
    private static String tag = "StepCount";
    private int n;
    final private double Data[][] = new double[StepData.ClassifierNum][MaxPointNum];
    private double Time[] = new double[MaxPointNum];
    private double Sumt;
    ...
    public Step(final double[][] DataIn,int Ending, int length, int count, long[] TimeSeries, double MagMean,double ButterPeak){
        // They should be calculated in for(int j).
//        super();
        int ArrayLen = SensorActivity.SavedPointNum;
        ...
        StepArray.Steps[count] = AStep;
        if (count > StepArray.MaxStepsNum){
            StepArray.Circle = true;
            StepArray.CurrentStep = count % StepArray.MaxStepsNum;
        }
        else StepArray.CurrentStep = count;
        ...
        if ((count > this.StartStep) || (StepArray.Circle)) {
            ...
            for(i = 0;i <= 5;i++)
                if (!StepArray.Circle)
                    ...
                    temp += CurbWeight[i] * StepArray.Steps[(count - 5 + i) % MaxStep].AccMagEndPeak;// Weighted
            temp = temp * CurbThreshold;// Threshold of Curb, Acc magnitude
            Log.d(tag, count-2 + ":"  + temp);
            StepArray.Steps[(count - 2) % MaxStep].AccMagThreshold = temp;
            if (temp < StepArray.Steps[(count - 2) % MaxStep].AccMagEndPeak){
                EventNum = MyLib.EVENT_UPCURB;
                if (StepArray.Steps[(count - 1)% MaxStep].Event < EventNum) {
                    StepArray.Steps[(count - 1)% MaxStep].Event = EventNum;
                    StepArray.HMMSeq[(count - 1)% MaxStep] = EventNum;
                }
                if (StepArray.Steps[(count - 2)% MaxStep].Event < EventNum) {
                    StepArray.Steps[(count - 2)% MaxStep].Event = EventNum;
                    StepArray.HMMSeq[(count - 2)% MaxStep] = EventNum;
                }
            }
            else if (StepArray.Turned == 0) {
                temp = StepArray.Steps[count].AzimuthMean;// mean Azimuth of the steps
                tempT = StepArray.Steps[count - 3].AzimuthMean + StepArray.Steps[count - 4].AzimuthMean;
                tempT = tempT /2;// mean Azimuth of the previous 3-4 steps
                if (Math.abs(tempT - temp) > TurnThreshold){
                    EventNum = MyLib.EVENT_TURN;
                    if (StepArray.Steps[count - 1].Event < EventNum) {
                        StepArray.Steps[count - 1].Event = EventNum;
                        StepArray.HMMSeq[count - 1] = EventNum;
                    }
                    if (StepArray.Steps[count - 2].Event < EventNum) {
                        StepArray.Steps[count - 2].Event = EventNum;
                        StepArray.HMMSeq[count - 2] = EventNum;
                    }
                    StepArray.Turned = 2;
                }
            }
...
//        StepArray.Steps[count].Features = AStep.Features;
        if (count > 3) {
            StepArray.LogData(count - 2);

        }
        if ((count > 10) & StepArray.PreformML)
            StepArray.HMM(count);
    }
}
